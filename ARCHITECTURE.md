...
轮询请求本身超时/网络错误时，异步任务在 OpenList 端仍在继续，cloud-sync 会**持续轮询同一 taskID**（不会重新发起 Copy，避免重复传输）；`TASK_TIMEOUT_SECONDS` 只约束**单次** `Copy`/查询请求，不限制单任务总时长——只有任务进入终态、任务不存在（404）或父 context 取消才结束。404 时先查目标是否存在：存在→视为 synced；不存在→写 FAILED 且**不重新 Copy**（任务被取消/清理后不应被重新加回）。轮询期间读取 OpenList 返回的 `progress`，在 UI 上以"上传中/百分比"展示。任务被**手动取消**（state==4）同样视为终态失败：写 FAILED 且**不重新发起 Copy**。
...
| 任务失败 (state==7) | 重新发起 Copy（最多 3 次），仍失败写 FAILED |
| 任务被手动取消 (state==4) | 写 FAILED，**不重新发起 Copy** |
| 任务不存在 (404) | 先 `Exists`：存在→synced；否则写 FAILED，**不重新发起 Copy** |
