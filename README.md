...
- **Dashboard**：同步计数（**已处理 processed** = synced + cleaned、synced、syncing、failed、cleaned、unsynced）、OpenList 连通性、运行时长、watch 目录；「Run cleanup now」立即触发一次清理（遵循 `cleanup_dry_run`）；「Rescan state」把"已清理但本地文件仍在"（size/mtime 一致）的记录恢复为 `synced`，交回清理流程删除。
...
**任务默认关闭**：`tasks_enabled`/`TASKS_ENABLED` 默认 `false`，启动后只起 UI、不监听/上传/清理。在 Dashboard 点「启动任务」运行时开启（或配置里设 true）。**注意**：`tasks_enabled` 是**进程启动时**的初始状态——通过 Web UI「保存并重载」热重载**不会**启动/暂停任务（避免保存无关配置时误停任务），运行时启停请用 Dashboard 的「启动任务/暂停」。开启前可先跑**上传预检查**（只读扫描：列出将上传的文件，并调用 OpenList `/api/fs/get` 检测每个文件是否已在云盘上，报告写 `precheck.json`）。暂停期间仍可单条重试、批量重试与手动清理（走一次性执行路径，无需先启动任务）。
