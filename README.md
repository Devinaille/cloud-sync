...
**任务默认关闭**：`tasks_enabled`/`TASKS_ENABLED` 默认 `false`，启动后只起 UI、不监听/上传/清理。在 Dashboard 点「启动任务」运行时开启（或配置里设 true）。开启前可先跑**上传预检查**（只读扫描，列出将上传的文件并写 `precheck.json`）。暂停期间重试/清理返回 `409`。
